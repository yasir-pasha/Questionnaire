...
@extends('layouts.app')
@section('content')
    <div class="page-header col-sm-12 row">
        <h1 class="col-sm-6 row float-left row"> Edit Questionnaires
        </h1>
    ...
    <form method="post" action="{{route('questionnaire.update',[$questionnaire->id])}}">
        ...
        <div class="form-group"> <!-- Full Name -->
            ...
            <input type="text" class="form-control" id="questionnaire_name" name="questionnaire_name"
                   placeholder="Enter Questionnaire Name" value="{{ $questionnaire->questionnaire_name }}">
        </div>
            ...
            <div class="row">
                <div class="col-sm-8">
                    <input type="text" class="form-control" id="duration" name="duration"
                           placeholder="Enter Duration" value="{{ $questionnaire->duration }}">
                ...
                <div class="col-sm-4">
                    <select name="duration_type" class="form-control float-right custom-select">
                        ...
                        <option value="hr" {{$questionnaire->duration_type == 'hr'?'selected':''}}>Hour</option>
                        <option value="min" {{$questionnaire->duration_type == 'min'?'selected':''}}>Minute</option>
                    </select>
                </div>
            </div>
            ...
            <div class="col-sm-12 row">
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="can_resume" id="can_resume_no"
                           value="1" {{$questionnaire->can_resume == '1'?'checked':''}}>
                ...
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="can_resume" id="can_resume_yes"
                           value="0" {{$questionnaire->can_resume == '0'?'checked':''}}>
            ...
            <div class="row">
                <div class="col-sm-4">
                    <select name="is_published" class="form-control float-right custom-select">
                        ...
                        <option value="1" {{$questionnaire->is_published == '1'?'selected':''}}>Yes</option>
                        <option value="0" {{$questionnaire->is_published == '0'?'selected':''}}>No</option>
                    </select>
                </div>
            </div>
